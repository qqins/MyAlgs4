...
/**
  ...
  和 Driver_Id 是 Users 表中 Users_Id 的外键。Status 是枚举类型，
...
SELECT
    ...
    round(
        SUM(
            CASE Status  -- 这里临时生成了一个字段，由status字段决定 若是完成字段值为0 反之为1  也就是是否被取消
...
FROM
    Trips t INNER JOIN Users u
    ON t.Client_Id = u.Users_Id and u.Banned='No'    -- 限定为非ban用户
